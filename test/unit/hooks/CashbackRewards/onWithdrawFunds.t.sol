...
contract OnWithdrawFundsTest is CashbackRewardsTest {
    function test_ownerCanWithdrawFunds(uint256 withdrawAmount) public {
        ...
        flywheel.withdrawFunds(
            ...
            address(usdc),
            abi.encode(
                Flywheel.Send({recipient: owner, amount: withdrawAmount, extraData: "", fallbackKey: bytes32(0)})
            )
        );
    ...
    function test_managerCannotWithdrawFunds(uint256 withdrawAmount) public {
        ...
        flywheel.withdrawFunds(
            ...
            address(usdc),
            abi.encode(
                Flywheel.Send({recipient: manager, amount: withdrawAmount, extraData: "", fallbackKey: bytes32(0)})
            )
        );
    }
}
