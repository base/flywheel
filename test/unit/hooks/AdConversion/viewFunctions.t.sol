...
abstract contract ViewFunctionsTest is AdConversionTestBase {
    // ========================================
    ...
    function test_campaignURI_returnsCorrectURI(address campaign, string memory expectedURI) public virtual;

    /// @dev Returns empty string for campaign with empty URI
    /// @param campaign Campaign address with empty URI
    function test_campaignURI_returnsEmptyString(address campaign) public virtual;
    ...
    function test_getConversionConfig_returnsCorrectConfig(address campaign, uint16 configId) public virtual;

    /// @dev Reverts when conversion config ID does not exist
    /// @param campaign Campaign address
    /// @param invalidConfigId Non-existent conversion config ID
    function test_getConversionConfig_revert_invalidId(address campaign, uint16 invalidConfigId) public virtual;
    ...
    function test_hasPublisherAllowlist_noAllowlist(address campaign) public virtual;

    /// @dev Returns true when campaign has allowlist
    /// @param campaign Campaign address with allowlist
    function test_hasPublisherAllowlist_withAllowlist(address campaign) public virtual;
    ...
    function test_isPublisherRefCodeAllowed_noAllowlist(address campaign, string memory anyRefCode) public virtual;

    /// @dev Returns true for allowed ref code when allowlist exists
    /// @param campaign Campaign address with allowlist
    /// @param allowedRefCode Publisher ref code in allowlist
    function test_isPublisherRefCodeAllowed_allowedCode(address campaign, string memory allowedRefCode)
        public
        virtual;
    ...
    function test_isPublisherRefCodeAllowed_disallowedCode(address campaign, string memory disallowedRefCode)
        public
        virtual;

    /// @dev Returns false for empty ref code when allowlist exists
    /// @param campaign Campaign address with allowlist
    function test_isPublisherRefCodeAllowed_emptyCode(address campaign) public virtual;
    ...
    /// @param campaign Campaign address
    function test_getConversionConfig_edge_configIdZero(address campaign) public virtual;

    /// @dev Handles maximum config ID
    /// @param campaign Campaign address
    /// @param maxConfigId Maximum valid config ID
    function test_getConversionConfig_edge_maximumConfigId(address campaign, uint16 maxConfigId) public virtual;

    /// @dev Handles disabled conversion config
    /// @param campaign Campaign address
    /// @param disabledConfigId Disabled conversion config ID
    function test_getConversionConfig_edge_disabledConfig(address campaign, uint16 disabledConfigId) public virtual;
}
