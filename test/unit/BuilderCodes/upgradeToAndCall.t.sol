...
contract UpgradeToAndCallTest is BuilderCodesTest {
    ...
    function test_upgradeToAndCall_success_canChangeEIP712DomainVersion() public {
        ...
        bytes memory v1Signature = _signRegistrationWithVersion(REGISTRAR_PK, testCode, testOwner, testPayoutAddress, deadline, "1");
        ...
        bytes memory oldV1Signature = _signRegistrationWithVersion(REGISTRAR_PK, newTestCode, testOwner, testPayoutAddress, deadline, "1");
        vm.expectRevert(BuilderCodes.Unauthorized.selector);
        builderCodes.registerWithSignature(newTestCode, testOwner, testPayoutAddress, deadline, registrar, oldV1Signature);
        
        // New signature (domain version "2") should work
        bytes memory v2Signature = _signRegistrationWithVersion(REGISTRAR_PK, newTestCode, testOwner, testPayoutAddress, deadline, "2");
