...
contract AdFlowTest is PublisherTestSetup {
    ...
    function test_endToEndAdFlow() public {
        ...
        assertEq(flywheel.fees(campaign, address(usdc), bytes32(bytes20(provider))), expectedFeeAmount * 2);

        // 6. Provider collects fees
        vm.startPrank(provider);
        flywheel.collectFees(campaign, address(usdc), abi.encode(provider));
        vm.stopPrank();
