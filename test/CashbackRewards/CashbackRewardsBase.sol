...
contract CashbackRewardsBase is Test {
    ...
    function _createCashbackCampaign() internal {
        ...
        bytes memory hookData = abi.encode(manager, CAMPAIGN_URI);
    ...
    function _setupInitialTokenBalances() internal {
        ...
        usdc.mint(cashbackCampaign, 1_000e6); // 1,000 USDC
    ...
    {
        return abi.encode(paymentInfo, rewardAmount);
    }
    ...
    function getRewardsInfo(AuthCaptureEscrow.PaymentInfo memory paymentInfo, address campaign)
        public
        view
        returns (CashbackRewards.RewardsInfo memory)
    {
        bytes32 paymentInfoHash = escrow.getHash(paymentInfo);
        (uint120 allocated, uint120 distributed) = cashbackRewards.rewardsInfo(paymentInfoHash, campaign);
        return CashbackRewards.RewardsInfo({allocated: allocated, distributed: distributed});
    }
