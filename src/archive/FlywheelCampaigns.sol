...
contract FlywheelCampaigns is Initializable, Ownable2StepUpgradeable, UUPSUpgradeable, IFlywheelCampaigns {
    ...
    modifier onlyValidCampaignAttribution(uint256 _campaignId) {
        ...
        if (
            campaignStatus == CampaignStatus.CREATED || campaignStatus == CampaignStatus.CAMPAIGN_READY
    ...
    function getConversionConfig(uint256 _campaignId, uint8 _conversionConfigId)
        external
        view
        returns (ConversionConfig memory)
    {
    ...
    function addConversionConfig(uint256 _campaignId, ConversionConfigInput memory _conversionConfig)
        external
    ...
    function deactivateConversionConfig(uint256 _campaignId, uint8 _conversionConfigId)
        external
        onlyAdvertiser(_campaignId)
    {
    ...
    function updateCampaignStatus(uint256 _campaignId, CampaignStatus _newStatus) external {
        ...
        bool isAttributionProvider =
            attributionProviders[campaigns[_campaignId].attributionProviderId].signerAddress == msg.sender;
        ...
        if (
            _newStatus == CampaignStatus.NONE || currentStatus == CampaignStatus.NONE || _newStatus == currentStatus
                || _newStatus == CampaignStatus.CREATED || currentStatus == CampaignStatus.COMPLETED // Cannot transition from COMPLETED
        ) {
    ...
    function isPublisherRefCodeAllowed(uint256 _campaignId, string memory _publisherRefCode)
        public
        view
        returns (bool)
    {
    ...
    function attributeOffchainEvents(uint256 _campaignId, OffchainEvent[] calldata _events)
        external
        onlyValidCampaignAttribution(_campaignId)
    {
    ...
    function attributeOnchainEvents(uint256 _campaignId, OnchainEvent[] calldata _events)
        external
        onlyValidCampaignAttribution(_campaignId)
    {
        uint256 totalNewAttributedAmount;
    ...
    function addAllowedPublisherRefCode(uint256 _campaignId, string memory _publisherRefCode)
        external
        onlyAdvertiser(_campaignId)
    {
    ...
    function _onlyValidEventType(uint256 _campaignId, uint8 _conversionConfigId, EventType _expectedEventType)
        private
        view
    {
    ...
    function _createConversionConfigs(uint256 campaignId, ConversionConfigInput[] memory conversionConfigs)
        private
    ...
    function _createConversionConfig(uint256 campaignId, uint8 conversionConfigId, ConversionConfigInput memory input)
        private
    {
        if (
            input.rewardType == RewardType.NONE || input.cadenceType == CadenceEventType.NONE
