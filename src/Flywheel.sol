...
contract Flywheel {
  ...
  function finalizeCampaign(address campaign) external {
    ...
    if (_isSponsor(campaign)) {
      ...
      if (oldStatus != CampaignStatus.CREATED && !(oldStatus == CampaignStatus.CLOSED && attributionDeadlinePassed)) {
  ...
  function attribute(address campaign, address payoutToken, bytes calldata attributionData) external {
    ...
    (Payout[] memory payouts, uint256 attributionFee) = AttributionHook(campaigns[campaign].hook).attribute(
      campaign,
      attributor,
      payoutToken,
      attributionData
