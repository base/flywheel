...
/// @notice Script for deploying the ReferralCodes contract
contract DeployReferralCodes is Script {
    /// @notice Deploys the ReferralCodes with proxy
    ...
    function run(address owner, address signerAddress, string memory uriPrefix) external returns (address) {
        ...
        bytes memory initData = abi.encodeCall(ReferralCodes.initialize, (owner, signerAddress, uriPrefix));
